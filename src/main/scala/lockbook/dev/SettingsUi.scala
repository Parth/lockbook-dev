...
class SettingsUi(settingsHelper: SettingsHelper, fileHelper: FileHelper) {
  ...
  def showView(): Unit = {
    ...
    if (result.isPresent) {
      ...
      restartDialog.getDialogPane.setContentText("Restart Lockbook for changes to take effect.")
  ...
  private def showViewHelper(dialog: Dialog[LockbookSettings], gridPane: GridPane): Unit = {
    ...
    autoLockIntField.setTextFormatter(new TextFormatter[String]((change: TextFormatter.Change) => {
      ...
      if (!(change.getText.length == 0 || autoLockIntField.getText.length == 0))
        if ((autoLockIntField.getText + change.getText).toShortOption.isEmpty || autoLockIntField.getText.length == 0 && change.getText == "0")
          change.setText("")
    ...
    gridPane.add(new Label("Automatically lock when idle"), 0, 2)
    gridPane.add(new Label("Enabled:"), 0, 3)
